...
const quiz = {
    0: {
        problem: [
            '통장에 돈을 넣는 것을 \'입금\', 통장에서 돈을 빼는 것을 \'출금\'이라고 한다?',
            '\'원금\'은 은행이 나에게 보관해줘서 고맙다고 주는 보너스 돈이다?',
            '\'이자\'는 내가 맡긴 돈(원금)이 낳은 \'아기 돈\'이라고 생각하면 쉽다?'
        ],
        answer: [
            true,
            false,
        ...
        explain: [
            '맞아요. 들어올 입(入)을 써서 입금, 나갈 출(出)을 써서 출금이라고 해요.',
            '아니에요. \'원금\'은 원래 내가 맡겼던 내 돈을 말해요. 보너스로 주는 돈은 \'이자\'예요.',
            '맞아요. 시간이 지나면 내가 맡긴 돈(원금)에 은행이 주는 보너스(이자)가 더해져서 돈이 불어나요.'
        ]
    },
    1: {
        problem: [
    ...
    7: {
        problem: [
            '금은 은행 예금처럼 가만히 가지고만 있어도 매달 이자가 붙는다?',
            '나라 경제가 어려워지거나 전쟁이 나는 등 세상이 불안할 때, 사람들은 금을 더 찾는다?',
            '금은 주식처럼 회사가 망해서 휴지 조각이 될 걱정이 없는 안전한 자산이다?'
        ],
        answer: [
            false,
            true,
        ...
        explain: [
            '아니에요. 금은 가지고 있다고 해서 이자를 주지 않아요. 나중에 금값이 올랐을 때 팔아야 돈을 벌 수 있어요.',
            '맞아요. 돈(화폐)의 가치가 떨어질까 봐 사람들은 변하지 않는 보물인 금을 사려고 해요.',
            '맞아요. 회사는 사라질 수 있지만, 금은 불에 타지도 않고 영원히 남아있어서 \'안전 자산\'이라고 불러요.'
        ]
    },
    8: {
        problem: [
            '부동산은 "아닐 부(不), 움직일 동(動), 낳을 산(産)"을 써서 \'움직이지 않는 재산\'(집, 땅, 건물)을 뜻한다?',
            '건물 주인이 되면, 다른 사람에게 건물을 빌려주고 매달 \'월세\'를 받을 수 있다?',
            '부동산은 슈퍼에서 과자를 팔듯이, 내가 돈이 급할 때 언제든지 바로 팔아서 현금으로 바꿀 수 있다?'
        ],
        answer: [
            true,
            true,
        ...
        explain: [
            '맞아요. 집이나 땅은 들고 다닐 수가 없죠? 그래서 움직이지 않는 자산이라고 불러요.',
            '맞아요. 은행 이자처럼, 내 집이나 건물을 남에게 빌려주면 꼬박꼬박 임대료(월세)를 받을 수 있어요.',
            '아니에요. 부동산은 가격이 비싸고 덩치가 커서, 사려는 사람을 찾는 데 시간이 아주 오래 걸릴 수도 있어요.'
        ]
    }
}
